...
Owned by `cohesion-locality`. Do not decide splits or joins from this section.
...
Do not split files by length alone. Whether parts share a change axis is `cohesion-locality`. This skill only structures a cut that has already been decided, or rejects a split that exists only to shorten a file.
