...
Upstream is `_reviewing-code-modularity-skill` (tracked copy — do not edit; update via `skillshare update`, then port diffs by hand, see `modularity-review/README.md`). Upstream `evals/trigger-queries.json` doubles as a routing regression set: every `should_trigger: true` query should land on exactly one of the two skills above.
